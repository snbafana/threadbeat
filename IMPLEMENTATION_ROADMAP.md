...
- Hosted checks passed: Railway `/health` returned 200 with runtime mode `pi-sdk`; `THREADBEAT_BASE_URL=https://your-railway-url npm run smoke:api` created and ran a hosted heartbeat.
- Cadence gate passed: a hosted test heartbeat persisted seven successful Pi/DeepSeek runs in Turso, then was set inactive to stop spend.
