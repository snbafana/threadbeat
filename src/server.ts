...
type WorkerSessionBranchNativeStatusNextAction = {
  surface: "worker_recovery" | "recover_next" | "operator" | "branch" | "result_inspection" | "control";
  action: string;
  reason: string;
  command: string[];
};
...
const readWorkerSessionControlPlaneStatus = async (
  ...
  recovery: {
    ...
    operatorLoops: WorkerSessionControlPlaneOperatorLoopHistoryStatus;
    resultReviewLoops: WorkerSessionControlPlaneResultReviewLoopHistoryStatus;
  ...
  const [
    ...
    listWorkerSessionControlPlaneAdvanceRecords(settings.projectRoot, name, {
      limit: Number.MAX_SAFE_INTEGER,
      detailCommands: ["branch_native_operator_loop"],
    }),
    listWorkerSessionControlPlaneAdvanceRecords(settings.projectRoot, name, {
      limit: Number.MAX_SAFE_INTEGER,
      detailCommands: ["branch_native_result_review_loop"],
    }),
    listWorkerSessionControlPlaneAdvanceRecords(settings.projectRoot, name, {
      limit: Number.MAX_SAFE_INTEGER,
  ...
  const operatorLoops = summarizeWorkerSessionControlPlaneOperatorLoopHistory(
    name,
    branchNativeOperatorLoopAttempts,
    lines,
  );
...
const summarizeWorkerSessionControlPlaneRecoverNextHistory = (
  ...
  const completedLoopAdvanceIds = new Set(records
    .filter((record) => record.detailCommand === "recover_next_loop")
    .map((record) => record.advanceId));
  const incompleteLoops = summarizeIncompleteRecoverNextLoops(sessionName, loopStepRecords, completedLoopAdvanceIds, lines);
  return {
...
const summarizeIncompleteRecoverNextLoops = (
  sessionName: string,
  loopStepRecords: Awaited<ReturnType<typeof listWorkerSessionControlPlaneAdvanceRecords>>,
  completedLoopAdvanceIds: Set<string>,
  lines: number,
): WorkerSessionControlPlaneRecoverNextHistoryStatus["incompleteLoops"] => {
  const grouped = new Map<string, Awaited<ReturnType<typeof listWorkerSessionControlPlaneAdvanceRecords>>>();
  for (const record of loopStepRecords) {
    const recovery = objectRecord(record.recovery);
    const loopAdvanceId = stringRecordField(recovery, "loopAdvanceId");
    if (!loopAdvanceId || completedLoopAdvanceIds.has(loopAdvanceId)) continue;
    const records = grouped.get(loopAdvanceId) ?? [];
    records.push(record);
    grouped.set(loopAdvanceId, records);
  }
  const summaries = [...grouped.entries()].map(([loopAdvanceId, records]) => {
    const sorted = [...records].sort((left, right) => {
      const leftIndex = numberRecordField(objectRecord(left.recovery), "stepIndex") ?? 0;
      const rightIndex = numberRecordField(objectRecord(right.recovery), "stepIndex") ?? 0;
      return rightIndex - leftIndex || right.observedAt.localeCompare(left.observedAt);
    });
    const last = sorted[0]!;
    const recovery = objectRecord(last.recovery);
    ...
    return {
      ...
      dryRun,
      lastStepIndex: numberRecordField(recovery, "stepIndex"),
...
const summarizeWorkerSessionControlPlaneOperatorLoopHistory = (
  sessionName: string,
  records: Awaited<ReturnType<typeof listWorkerSessionControlPlaneAdvanceRecords>>,
  lines: number,
): WorkerSessionControlPlaneOperatorLoopHistoryStatus => {
  const grouped = new Map<string, Awaited<ReturnType<typeof listWorkerSessionControlPlaneAdvanceRecords>>>();
  for (const record of records) {
    const recovery = objectRecord(record.recovery);
    const selected = objectRecord(record.selected);
  ...
  const resumableLoops = [...grouped.entries()].flatMap(([loopAdvanceId, loopRecords]) => {
    ...
    const recovery = objectRecord(latest.recovery);
    const stoppedReason = stringRecordField(recovery, "stoppedReason");
    if (stoppedReason === "idle" || stoppedReason === "operator_failed") return [];
    const maxSteps = numberRecordField(recovery, "maxSteps");
    const intervalMs = numberRecordField(recovery, "intervalMs");
    const maxCycles = numberRecordField(recovery, "maxCycles");
    const cycleIntervalMs = numberRecordField(recovery, "cycleIntervalMs");
    ...
    return [{
      ...
      resumeCommand: [
        ...
        "--max-steps", String(maxSteps ?? 10),
        "--interval-ms", String(intervalMs ?? 2000),
        "--max-cycles", String(maxCycles ?? 1),
        "--cycle-interval-ms", String(cycleIntervalMs ?? 2000),
        "--lines", String(loopLines ?? 5),
        dryRun ? "--dry-run" : "--confirm",
      ],
      inspectLatestCommand: ["npm", "run", "cli", "--", "runs", "session-control-plane-advances", sessionName, "--server", "--advance", latest.advanceId],
      inspectHistoryCommand: ["npm", "run", "cli", "--", "runs", "session-control-plane-advances", sessionName, "--server", "--loop-advance-id", loopAdvanceId, "--detail-command", "branch_native_operator_loop"],
      executeResumeCommand: [
        "npm", "run", "cli", "--", "runs", "session-branch-native-next", sessionName, "--server",
        "--operate", "--until-empty", "--resume-loop", loopAdvanceId,
        "--max-steps", String(maxSteps ?? 10),
        "--interval-ms", String(intervalMs ?? 2000),
        "--max-cycles", String(maxCycles ?? 1),
        "--cycle-interval-ms", String(cycleIntervalMs ?? 2000),
...
const summarizeWorkerSessionControlPlaneResultReviewLoopHistory = (
  ...
  const loopSummaries = [...grouped.entries()].flatMap(([loopAdvanceId, loopRecords]) => {
    ...
    const intervalMs = numberRecordField(recovery, "intervalMs");
    const remainingPending = numberRecordField(recovery, "remainingPending");
    const totalProcessed = loopRecords.reduce((total, record) => {
      const recordRecovery = objectRecord(record.recovery);
      return total + (numberRecordField(recordRecovery, "processed") ?? 0);
    }, 0);
    const dryRun = sorted.every((record) => record.dryRun);
    const resumeCommand = [
      "npm", "run", "cli", "--", "runs", "session-branch-native-next", sessionName, "--server",
...
type WorkerSessionControlPlaneAlertDetails =
  ...
  | {
    ...
    advance: Awaited<ReturnType<typeof listWorkerSessionControlPlaneAdvanceRecords>>[number];
    commands: {
  ...
  | {
    ...
    loop: WorkerSessionControlPlaneRecoverNextHistoryStatus["incompleteLoops"]["recent"][number];
    commands: {
      resumeLoop: string[];
...
const readWorkerSessionControlPlaneAlertDetails = async (
  ...
  if (alert.surface === "status_watch" && alert.advanceId) {
...
const readWorkerSessionControlPlaneAlerts = async (
  ...
  const allAlerts: WorkerSessionControlPlaneAlert[] = [
    ...
    ...failedStatusWatchExecutions.map((record) => {
      const selected = objectRecord(record.selected);
      return {
        surface: "status_watch" as const,
        severity: "error" as const,
        reason: "failed_status_watch_execution",
        count: 1,
        command: ["npm", "run", "cli", "--", "runs", "session-control-plane-advances", name, "--server", "--status-watch-executions", "--advance", record.advanceId],
        advanceId: record.advanceId,
        action: "inspect_status_watch_execution",
        ...(stringRecordField(selected, "workerId") ? { workerId: stringRecordField(selected, "workerId") ?? undefined } : {}),
        ...(stringRecordField(selected, "applyId") ? { applyId: stringRecordField(selected, "applyId") ?? undefined } : {}),
        ...(stringRecordField(selected, "executionId") ? { executionId: stringRecordField(selected, "executionId") ?? undefined } : {}),
      };
    }),
...
const selectWorkerSessionBranchNativeStatusNext = (
  ...
  const operatorLoop = input.operatorLoops.resumableLoops.recent[0];
  if (operatorLoop) {
    return {
      surface: "operator",
  ...
  const resultReviewLoop = input.resultReviewLoops.resumableLoops.recent[0];
  if (resultReviewLoop) {
    return {
