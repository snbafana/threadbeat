...
async function assertControlPlaneTopologyEnsure(baseUrl: string, sessionName: string): Promise<void> {
  const coreOnlyAdvanceWorkerId = "detached-smoke-ensure-topology-core-advance-worker";
  const coreOnlyTickWorkerId = "detached-smoke-ensure-topology-core-tick-worker";
  const applyWorkerId = "detached-smoke-ensure-topology-apply-worker";
  const drainWorkerId = "detached-smoke-ensure-topology-drain-worker";
  const coreOnlyArgs = [
    "runs",
    "ensure-control-plane-topology",
    sessionName,
    "--server",
    "--advance-worker-id",
    coreOnlyAdvanceWorkerId,
    "--tick-worker-id",
    coreOnlyTickWorkerId,
    "--worker-dry-run",
    "1",
    "--max-steps",
    "50",
    ...
    drainWorkerId,
    "--apply-id",
    "detached-smoke-ensure-topology-empty-apply",
    "--apply-action",
    "inspect_drain_continuation_resets",
    "--max-actions",
    "1",
  ...
  const dryRunTopologyEnsure = await cliJson<{
    ok?: true;
    session: string;
    dryRun: boolean;
    confirmed: boolean;
    ...
    passed: boolean | null;
    plan: { expected: number; coreExpected: number; mutationExpected: number; actionable: number; blocked: number; skippedMutationActions: number; commands: string[][] };
    core: { plan: { actionable: number }; executed: unknown[] };
    mutation: { plan: { actionable: number }; executed: unknown[] };
    checks: { corePassed: boolean | null; mutationPassed: boolean | null; mutationSkipped: boolean; coreExecutedCount: number | null; mutationExecutedCount: number | null; beforeReady: boolean; afterReady: boolean | null };
  }>(baseUrl, [...coreOnlyArgs, "--dry-run"]);
  assert.equal(dryRunTopologyEnsure.ok, true);
  assert.equal(dryRunTopologyEnsure.session, sessionName);
  assert.equal(dryRunTopologyEnsure.dryRun, true);
  assert.equal(dryRunTopologyEnsure.confirmed, false);
  assert.equal(dryRunTopologyEnsure.includeMutationWorkers, false);
  assert.equal(dryRunTopologyEnsure.mutationSkipped, false);
  assert.equal(dryRunTopologyEnsure.passed, null);
  assert.equal(dryRunTopologyEnsure.plan.expected, 4);
  assert.equal(dryRunTopologyEnsure.plan.coreExpected, 2);
  assert.equal(dryRunTopologyEnsure.plan.mutationExpected, 2);
  assert.equal(dryRunTopologyEnsure.plan.actionable, 4);
  assert.equal(dryRunTopologyEnsure.plan.blocked, 0);
  assert.equal(dryRunTopologyEnsure.plan.skippedMutationActions, 0);
  assert.equal(dryRunTopologyEnsure.plan.commands.length, 4);
  assert.equal(dryRunTopologyEnsure.core.executed.length, 0);
  assert.equal(dryRunTopologyEnsure.mutation.executed.length, 0);
  assert.equal(dryRunTopologyEnsure.checks.afterReady, null);

  const coreOnlyTopologyEnsure = await cliJson<{
    ok?: true;
    dryRun: boolean;
    confirmed: boolean;
    includeMutationWorkers: boolean;
    mutationSkipped: boolean;
    passed: boolean;
    plan: { actionable: number; skippedMutationActions: number; commands: string[][] };
    core: { passed: boolean; executed: Array<{ kind: string; workerId: string; actionResult: string | null }> };
    mutation: { passed: null; executed: unknown[] };
    checks: { corePassed: boolean; mutationPassed: null; mutationSkipped: boolean; coreExecutedCount: number; mutationExecutedCount: null; afterReady: boolean; afterRunningCount: number };
  }>(baseUrl, [...coreOnlyArgs, "--confirm"]);
  assert.equal(coreOnlyTopologyEnsure.ok, true);
  assert.equal(coreOnlyTopologyEnsure.dryRun, false);
  assert.equal(coreOnlyTopologyEnsure.confirmed, true);
  assert.equal(coreOnlyTopologyEnsure.includeMutationWorkers, false);
  assert.equal(coreOnlyTopologyEnsure.mutationSkipped, true);
  assert.equal(coreOnlyTopologyEnsure.passed, true);
  assert.equal(coreOnlyTopologyEnsure.plan.actionable, 2);
  assert.equal(coreOnlyTopologyEnsure.plan.skippedMutationActions, 2);
  assert.equal(coreOnlyTopologyEnsure.plan.commands.length, 2);
  assert.equal(coreOnlyTopologyEnsure.core.passed, true);
  assert.equal(coreOnlyTopologyEnsure.core.executed.length, 2);
  assert.deepEqual(
    coreOnlyTopologyEnsure.core.executed.map((step) => [step.kind, step.workerId, step.actionResult]),
    [
      ["control_plane_advance", coreOnlyAdvanceWorkerId, "started"],
      ["control_plane_tick", coreOnlyTickWorkerId, "started"],
    ],
  );
  assert.equal(coreOnlyTopologyEnsure.mutation.executed.length, 0);
  assert.equal(coreOnlyTopologyEnsure.checks.mutationSkipped, true);
  ...
  const allWorkerAdvanceWorkerId = "detached-smoke-ensure-topology-all-advance-worker";
  const allWorkerTickWorkerId = "detached-smoke-ensure-topology-all-tick-worker";
  const allWorkerApplyWorkerId = "detached-smoke-ensure-topology-all-apply-worker";
  const allWorkerDrainWorkerId = "detached-smoke-ensure-topology-all-drain-worker";
  const allWorkerArgs = [
    "runs",
    "ensure-control-plane-topology",
    sessionName,
    "--server",
    "--advance-worker-id",
    allWorkerAdvanceWorkerId,
    "--tick-worker-id",
    allWorkerTickWorkerId,
    "--worker-dry-run",
    "1",
    "--max-steps",
    "50",
    ...
    "1000",
    "--apply-worker-id",
    allWorkerApplyWorkerId,
    "--drain-worker-id",
    allWorkerDrainWorkerId,
    "--apply-id",
    "detached-smoke-ensure-topology-all-empty-apply",
    "--apply-action",
    "inspect_drain_continuation_resets",
    "--max-actions",
    "1",
    ...
    "--lines",
    "5",
    "--include-mutation-workers",
  ];
  const allWorkerTopologyEnsure = await cliJson<{
    ok?: true;
    includeMutationWorkers: boolean;
    mutationSkipped: boolean;
    passed: boolean;
    plan: { actionable: number; skippedMutationActions: number; commands: string[][] };
    core: { passed: boolean; executed: Array<{ kind: string; workerId: string; actionResult: string | null }> };
    mutation: { passed: boolean; executed: Array<{ kind: string; workerId: string; actionResult: string | null }> };
    checks: { coreExecutedCount: number; mutationExecutedCount: number; afterRunningCount: number };
  }>(baseUrl, [...allWorkerArgs, "--confirm"]);
  assert.equal(allWorkerTopologyEnsure.ok, true);
  assert.equal(allWorkerTopologyEnsure.includeMutationWorkers, true);
  assert.equal(allWorkerTopologyEnsure.mutationSkipped, false);
  assert.equal(allWorkerTopologyEnsure.passed, true);
  assert.equal(allWorkerTopologyEnsure.plan.actionable, 4);
  assert.equal(allWorkerTopologyEnsure.plan.skippedMutationActions, 0);
  assert.equal(allWorkerTopologyEnsure.plan.commands.length, 4);
  assert.equal(allWorkerTopologyEnsure.core.passed, true);
  assert.equal(allWorkerTopologyEnsure.mutation.passed, true);
  assert.equal(allWorkerTopologyEnsure.core.executed.length, 2);
  assert.equal(allWorkerTopologyEnsure.mutation.executed.length, 2);
  assert.deepEqual(
    allWorkerTopologyEnsure.mutation.executed.map((step) => [step.kind, step.workerId, step.actionResult]),
    [
      ["apply_action", allWorkerApplyWorkerId, "started"],
      ["drain", allWorkerDrainWorkerId, "started"],
    ],
  );
  assert.equal(allWorkerTopologyEnsure.checks.coreExecutedCount, 2);
  assert.equal(allWorkerTopologyEnsure.checks.mutationExecutedCount, 2);
  assert.ok(allWorkerTopologyEnsure.checks.afterRunningCount >= 1);
  await cliJson(baseUrl, [
    "runs",
    "stop-control-plane-advance-workers",
    sessionName,
...
try {
  ...
  const controlPlaneStatus = await cliJson<{
    ...
    results: {
      ...
      reviews: {
        count: number;
        recent: Array<{ reviewId: string; runId: string; resultCommit: string; action: string }>;
      };
    };
    staleRuns: {
  ...
  assert.equal(controlPlaneStatus.results.counts.pending, controlPlaneStatus.results.actions.review_result);
  ...
  const resultReviewDryRun = await cliJson<{
    ...
    dryRun: boolean;
    recorded: boolean;
    review: { reviewId: string; action: string; runId: string; resultCommit: string; command: string[] };
  }>(baseUrl, [
    "runs",
  ...
  assert.equal(resultReviewDryRun.ok, true);
  assert.equal(resultReviewDryRun.session, sessionName);
  assert.equal(resultReviewDryRun.dryRun, true);
  assert.equal(resultReviewDryRun.recorded, false);
  assert.equal(resultReviewDryRun.review.reviewId, "dry-run");
  assert.equal(resultReviewDryRun.review.action, "reviewed");
  assert.equal(resultReviewDryRun.review.runId, controlPlaneResultPlan.run.id);
  assert.equal(resultReviewDryRun.review.resultCommit, controlPlaneResultCommit);
  assert.equal(resultReviewDryRun.review.command.join(" "), `npm run cli -- runs review ${controlPlaneResultPlan.run.id} --checkout-dir ./checkouts/${sessionName}-control-plane-results/${controlPlaneResultPlan.run.id}`);
  const resultReview = await cliJson<{
    ok?: true;
    session: string;
    dryRun: boolean;
    recorded: boolean;
  ...
  assert.equal(resultReview.ok, true);
  assert.equal(resultReview.session, sessionName);
  assert.equal(resultReview.recorded, true);
  assert.equal(resultReview.dryRun, false);
  assert.ok(resultReview.reviewPath.includes(`.threadbeat/worker-sessions/result-reviews/${sessionName}/`));
  assert.equal(resultReview.review.action, "reviewed");
  assert.equal(resultReview.review.runId, controlPlaneResultPlan.run.id);
  assert.equal(resultReview.review.resultCommit, controlPlaneResultCommit);
  assert.equal(resultReview.review.reviewedBy, "detached-smoke");
  const listedResultReviews = await cliJson<{
    ok?: true;
    session: string;
    count: number;
  ...
  assert.equal(listedResultReviews.ok, true);
  assert.equal(listedResultReviews.count, 1);
  assert.equal(listedResultReviews.reviews[0]?.reviewId, resultReview.review.reviewId);
  ...
  assert.equal(controlPlaneStatusAfterResultReview.results.counts.resultCommits, controlPlaneStatus.results.counts.resultCommits);
  assert.equal(controlPlaneStatusAfterResultReview.results.counts.reviewed, controlPlaneStatus.results.counts.reviewed + 1);
  ...
  assert.equal(resultReviewTimeline.ok, true);
  assert.deepEqual(resultReviewTimeline.filter.sources, ["result_review"]);
  assert.deepEqual(resultReviewTimeline.filter.events, ["result_review_recorded"]);
  assert.deepEqual(resultReviewTimeline.filter.statuses, ["reviewed"]);
  assert.deepEqual(resultReviewTimeline.filter.runIds, [controlPlaneResultPlan.run.id]);
  assert.equal(resultReviewTimeline.counts.result_review_recorded, 1);
  assert.equal(resultReviewTimeline.count, 1);
  assert.equal(resultReviewTimeline.events[0]?.reviewId, resultReview.review.reviewId);
  assert.equal(resultReviewTimeline.events[0]?.source, "result_review");
  assert.equal(resultReviewTimeline.events[0]?.event, "result_review_recorded");
  assert.equal(resultReviewTimeline.events[0]?.status, "reviewed");
  assert.ok(resultReviewTimeline.events[0]?.runIds?.includes(controlPlaneResultPlan.run.id));
  assert.ok(resultReviewTimeline.events[0]?.branchNames?.includes(controlPlaneResultPlan.plan.branchName));
  assert.equal(resultReviewTimeline.events[0]?.resultCommit, controlPlaneResultCommit);
  assert.equal(resultReviewTimeline.events[0]?.reviewedBy, "detached-smoke");
  assert.equal(resultReviewTimeline.events[0]?.command?.join(" "), `npm run cli -- runs review ${controlPlaneResultPlan.run.id} --checkout-dir ./checkouts/${sessionName}-control-plane-results/${controlPlaneResultPlan.run.id}`);
  const resultReviewTimelineCommands = await cliJson<{
    ok?: true;
    commands: Array<{ action: string; reviewId: string | null; runIds: string[]; command: string[] }>;
  }>(baseUrl, [
    "runs",
    "session-control-plane-timeline",
    sessionName,
  ...
  assert.equal(resultReviewTimelineCommands.ok, true);
  assert.ok(resultReviewTimelineCommands.commands.some((command) => (
    command.action === "inspect_result_review"
    && command.reviewId === resultReview.review.reviewId
    && command.runIds.includes(controlPlaneResultPlan.run.id)
    && command.command.join(" ") === `npm run cli -- runs session-result-reviews ${sessionName} --server --review ${resultReview.review.reviewId} --run ${controlPlaneResultPlan.run.id} --limit 20`
  )));
  assert.equal(
  ...
  const controlPlaneWorkers = await cliJson<{
    ...
    summary: {
      ...
      applyAction: { total: number; stopped: number; retired: number; restartable: number };
      drain: { total: number; stopped: number; retired: number; restartable: number };
    };
    workers: Array<{
      kind: "control_plane_advance" | "control_plane_tick" | "apply_action" | "drain";
      workerId: string | null;
      state: string | null;
      restartable: boolean;
  ...
  assert.ok(controlPlaneWorkers.summary.applyAction.total >= 2);
  assert.ok(controlPlaneWorkers.summary.applyAction.stopped >= 1);
  assert.ok(controlPlaneWorkers.summary.applyAction.retired >= 1);
  assert.ok(controlPlaneWorkers.summary.applyAction.restartable >= 1);
  assert.ok(controlPlaneWorkers.summary.drain.total >= 1);
  assert.ok(controlPlaneWorkers.summary.drain.stopped >= 1);
  assert.ok(controlPlaneWorkers.summary.drain.restartable >= 1);
  ...
  const aggregateApplyActionWorker = controlPlaneWorkers.workers.find((worker) => worker.workerId === "detached-smoke-apply-action-worker");
  assert.equal(aggregateApplyActionWorker?.kind, "apply_action");
  assert.equal(aggregateApplyActionWorker?.state, "stopped");
  assert.equal(aggregateApplyActionWorker?.restartable, true);
  assert.equal(
    aggregateApplyActionWorker?.commands?.restart.join(" "),
    `npm run cli -- runs restart-apply-action-workers ${sessionName} --server --worker-id detached-smoke-apply-action-worker --include-retired`,
  );
  const aggregateDrainWorker = controlPlaneWorkers.workers.find((worker) => worker.workerId === serverDrainWorkerId);
  assert.equal(aggregateDrainWorker?.kind, "drain");
  assert.equal(aggregateDrainWorker?.state, "stopped");
  assert.equal(aggregateDrainWorker?.restartable, true);
  assert.equal(
    aggregateDrainWorker?.commands?.restart.join(" "),
    `npm run cli -- runs restart-drain-workers ${sessionName} --server --worker-id ${serverDrainWorkerId} --include-retired`,
  );
  ...
  assert.ok(controlPlaneWorkers.nextSteps.some((step) => (
    step.kind === "apply_action"
    && step.workerId === "detached-smoke-apply-action-worker"
    && step.action === "restart_apply_action_worker"
    && step.reason === "stopped_apply_action_worker"
    && step.command.join(" ") === `npm run cli -- runs restart-apply-action-workers ${sessionName} --server --worker-id detached-smoke-apply-action-worker`
  )));
  assert.ok(controlPlaneWorkers.nextSteps.some((step) => (
    step.kind === "drain"
    && step.workerId === serverDrainWorkerId
    && step.action === "restart_drain_worker"
    && step.reason === "stopped_drain_worker"
    && step.command.join(" ") === `npm run cli -- runs restart-drain-workers ${sessionName} --server --worker-id ${serverDrainWorkerId}`
  )));
  assert.equal(
  ...
  assert.ok(controlPlaneWorkerCommands.includes(
    `npm run cli -- runs restart-apply-action-workers ${sessionName} --server --worker-id detached-smoke-apply-action-worker`,
  ));
  assert.ok(controlPlaneWorkerCommands.includes(
    `npm run cli -- runs restart-drain-workers ${sessionName} --server --worker-id ${serverDrainWorkerId}`,
  ));
  ...
  await assertControlPlaneTopologyEnsure(baseUrl, sessionName);
