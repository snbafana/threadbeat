...
try {
  ...
  const openDrainQueuedContinuation = await cliJson<{
    session: string;
    ...
    continuation: {
      ...
      drains: Array<{ prefix: string; nextApplyId: string; command: string[]; exitCode: number | null }>;
    };
  }>(baseUrl, [
    "runs",
  ...
  assert.equal(await fileExists(openDrainQueuedContinuation.continuationPath), true);
  assert.equal(openDrainQueuedContinuation.session, detachedWorkerSessionName);
  assert.equal(openDrainQueuedContinuation.continuation.status, "queued");
  assert.equal(openDrainQueuedContinuation.continuation.dryRun, true);
  assert.equal(openDrainQueuedContinuation.continuation.readinessSource, "server");
  assert.deepEqual(openDrainQueuedContinuation.continuation.readinessCounts, openDrainApi.counts);
  assert.deepEqual(openDrainQueuedContinuation.continuation.continueDrains, {
    dryRun: true,
    selected: 1,
    succeeded: 0,
    failed: 0,
  });
  assert.equal(openDrainQueuedContinuation.continuation.drains[0].prefix, openDrainPrefix);
  assert.equal(openDrainQueuedContinuation.continuation.drains[0].nextApplyId, `${openDrainPrefix}-002`);
  assert.equal(openDrainQueuedContinuation.continuation.drains[0].exitCode, null);
  assert.deepEqual(openDrainQueuedContinuation.continuation.drains[0].command, [
  ...
  assert.equal(openDrainExecutedQueuedContinuation.continuation.dryRun, true);
  assert.ok(openDrainExecutedQueuedContinuation.continuation.startedAt);
  assert.ok(openDrainExecutedQueuedContinuation.continuation.completedAt);
  assert.deepEqual(openDrainExecutedQueuedContinuation.continuation.readinessCounts, openDrainApi.counts);
  assert.deepEqual(openDrainExecutedQueuedContinuation.continuation.continueDrains, {
    dryRun: true,
    selected: 1,
    succeeded: 1,
    failed: 0,
  });
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].prefix, openDrainPrefix);
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].nextApplyId, `${openDrainPrefix}-002`);
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].exitCode, 0);
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].output?.applyIdPrefix, openDrainPrefix);
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].output?.continuePrefix, openDrainPrefix);
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].output?.untilEmpty?.startPoll, 2);
  assert.equal(openDrainExecutedQueuedContinuation.continuation.drains[0].output?.untilEmpty?.maxPolls, 3);
  const openDrainQueuedBatchA = await cliJson<typeof openDrainQueuedContinuation>(baseUrl, [
    "runs",
    "session-drain-continuations",
    detachedWorkerSessionName,
    "--queue",
  ...
  const openDrainQueuedBatchB = await cliJson<typeof openDrainQueuedContinuation>(baseUrl, [
    "runs",
    "session-drain-continuations",
    detachedWorkerSessionName,
    "--queue",
  ...
  assert.equal(openDrainExecutedQueuedBatch.session, detachedWorkerSessionName);
  assert.equal(openDrainExecutedQueuedBatch.executed, 2);
  assert.equal(openDrainExecutedQueuedBatch.remainingQueued, 0);
  assert.deepEqual(
    openDrainExecutedQueuedBatch.continuations.map((item) => item.continuationId).sort(),
    [
      openDrainQueuedBatchA.continuation.continuationId,
      openDrainQueuedBatchB.continuation.continuationId,
    ].sort(),
  ...
  const freshRunningReset = await cliJson<{
    session: string;
    inspected: number;
    running: number;
    resetCount: number;
  ...
  assert.equal(freshRunningReset.session, detachedWorkerSessionName);
  assert.equal(freshRunningReset.running, 1);
  ...
  assert.equal(staleRunningReset.skippedRunning, 0);
  assert.equal(staleRunningReset.continuations[0]?.continuationId, staleRunningContinuation.continuation.continuationId);
  assert.equal(staleRunningReset.continuations[0]?.status, "queued");
  ...
  const staleResetExecuted = await cliJson<typeof openDrainExecutedQueuedContinuation>(baseUrl, [
    "runs",
    "session-drain-continuations",
    detachedWorkerSessionName,
    "--execute-next",
  ]);
  assert.equal(staleResetExecuted.continuation.continuationId, staleRunningContinuation.continuation.continuationId);
  assert.equal(staleResetExecuted.continuation.status, "executed");
  assert.equal(staleResetExecuted.continuation.continueDrains.succeeded, 1);
  ...
  const openDrainContinuationsResponse = await app.inject({
    method: "GET",
    url: `/api/worker-sessions/${detachedWorkerSessionName}/apply-drain-continuations?limit=10`,
  });
  ...
  assert.ok(openDrainContinuations.count >= 5);
  ...
  assert.deepEqual(executedContinuation.readinessCounts, openDrainApi.counts);
  assert.deepEqual(executedContinuation.continueDrains, openDrainContinueDrainsPreview.continueDrains);
  assert.equal(executedContinuation.drains[0].prefix, openDrainPrefix);
  assert.equal(executedContinuation.drains[0].nextApplyId, `${openDrainPrefix}-002`);
  assert.equal(executedContinuation.drains[0].exitCode, 0);
  ...
  }>(baseUrl, ["runs", "session-drain-continuations", detachedWorkerSessionName, "--limit", "10"]);
  assert.equal(openDrainContinuationsCli.session, detachedWorkerSessionName);
  assert.ok(openDrainContinuationsCli.count >= 5);
