...
try {
  ...
  assert.equal(advanceAll.count, 2);
...
type WorkerNextResponse = {
  ...
  nextSteps: Array<{ workerId: string; command: string[] }>;
};
...
async function writeTickWorker(workerId: string): Promise<void> {
