...
next-action commands. The summary also reports recent status-watch executions
and the command to inspect their durable handoff records. Add `--watch --max-polls <n> --interval-ms <ms>` to keep
...
selected command immediately after the watch finds it and persist that handoff
...
`GET /api/worker-sessions/:name/control-plane-ticks`. Each listed tick includes
a bounded `decision` summary with planned, executed, skipped, and not-planned
surfaces plus before/after ready counts for stale-run recovery, branch recovery,
apply actions, and drain continuations, so long-running control-plane loops can
be audited without opening every raw tick file. Use `runs
...
surface/action, count, reason, command, and exit code.
...
apply counts, alongside worker, branch recovery, and recent branch-recovery
execution counts. JSON watch snapshots also include the recent branch-recovery
execution records so operators can see which branches were already requeued
without leaving the live watch view. Generated result-review commands use the
watch checkout directory. Add `--until-empty`
...
the CLI. Use `runs session-result-inspections <name> --server --format text`
...
includes those same branch-native action commands. It also surfaces recent
branch-recovery executions with their inspect commands, the same next-result
and review-next commands when pending result commits exist, plus
...
`--commands-only --format shell` to print only the recommended review/resume
commands from that server-backed branch queue, and use
